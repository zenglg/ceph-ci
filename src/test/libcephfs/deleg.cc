...
static void simple_deleg_test(struct ceph_mount_info *cmount, struct ceph_mount_info *tcmount)
{
  ...
  ASSERT_EQ(ceph_ll_delegation(cmount, fh, F_WRLCK, dummy_deleg_cb, &recalled), 0);
  std::thread breaker1(breaker_func, tcmount, filename, O_RDWR);
  while (!recalled.load())
    usleep(1000);
  ASSERT_EQ(ceph_ll_delegation(cmount, fh, F_UNLCK, dummy_deleg_cb, &recalled), 0);
  breaker1.join();
  ...
  recalled.store(false);
  ASSERT_EQ(ceph_ll_delegation(cmount, fh, F_RDLCK, dummy_deleg_cb, &recalled), 0);
