...
void Objecter::_linger_reconnect(LingerOp *info, int r)
{
  ...
  if (r < 0) {
	...
	finisher->enqueue(in_place_t<CB_DoWatchError>{}, this, info, r);
      }
    }
    wl.unlock();
  }
...
void Objecter::handle_watch_notify(MWatchNotify *m)
{
  ...
  if (m->opcode == CEPH_WATCH_EVENT_DISCONNECT) {
    if (!info->last_error) {
      ...
      if (info->watch_context) {
	finisher->enqueue(in_place_t<CB_DoWatchError>{}, this, info, -ENOTCONN);
      }
    }
    ...
    finisher->enqueue(in_place_t<CB_DoWatchNotify>{}, this, info, m);
  }
}
