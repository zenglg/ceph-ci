...
def allocate_osd_id(
    ...
    if lockbox_osd_id:
        ...
        shutil.move(os.path.join(lockbox_path, 'keyring'),
    ...
    check_osd_id(osd_id)
    secrets.write_osd_keyring(path, osd_id)
    return osd_id
...
class Secrets(object):
    ...
    def write_osd_keyring(self, path, osd_id):
        command_check_call(
            [
                'ceph-authtool', os.path.join(path, 'keyring'),
                '--create-keyring',
                '--name', 'osd.' + str(osd_id),
